...
//点击树
function changeContent(rowId){
    console.log("点击树执行了")
    let typeBook={
        url:ip+"book-type/"+rowId,
        method:"GET"
    }
    axios(typeBook).then(data=>{
        ...
        booksJson.forEach(item=>{
            var $row = JSON.stringify(item).replace(/\"/g,"'");
            let itemDesc = item.book.synopsis
            nHtml+='<div class="oneEle"><img src="'+item.book.cover+'" class="img ib mr-1 menuBook cursor" onclick="toBook('+$row+')">'
            nHtml+='<div class="ib bookInfo">'
            nHtml+='<div class="title mb-1">书籍名称：'+item.book.name+'</div>'
            nHtml+='<div class="author mb-1">作者：'+item.book.author+'</div>'
